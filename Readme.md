...
For the battery/sleep version, install from **Library Manager**: search **"FlashStorage"** → Install (by cmaglie, for SAMD21 flash). RTCZero is not used in v3.0 (timeless model).
...
The sketch uses a **batched log flow** (v3.0 Timeless Resilience):
...
**Payload format (v3.0):** **FPort 10** (300 s) / **FPort 20** (10800 s): 4-byte header `[vbat_hi, vbat_lo]`, `[flags]` (bit0 = watchdog, bit1 = cold boot), `[sequence]` (0–255), then `[temp_hi, temp_lo]`×n. Newest reading first. **FPort 2** = HELLO WORLD. **FPort 1** / **FPort 4** = legacy (decoder still supports them). Temperature: 0–3000 = centidegrees; 0xFFFD/0xFFFE/0xFFFF = over/under/error. Use [`ttn-decoder-batch.js`](ttn-decoder-batch.js).
