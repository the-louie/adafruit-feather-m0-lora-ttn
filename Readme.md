...
Time is kept by the RTC (RTCZero) across sleep; epoch is persisted in flash and restored after power loss. **RTC is synced from the network** via LoRaWAN DeviceTimeReq/DeviceTimeAns (first after EV_JOINED, then when last sync >12 h ago). **Gateway:** DeviceTimeReq/DeviceTimeAns requires LoRaWAN 1.0.3+ and gateway support; 20% clock error is set for Feather M0 to improve RX2 reception of DeviceTimeAns. If MAC time sync fails for **>4 hours**, the firmware sends a batch with **baseTick=0**; the decoder uses **received_at** for 5-minute sample alignment. **Implicit catch-up:** first successful time sync with pending RAM data triggers a send in 10 s to drain backlog. Session is saved on join and restored on wake. **Join-first power isolation:** DS18B20/sensors are initialised only after EV_JOINED (or when restoring a session), so OneWire stays inert during join. **PROD:** Serial and all `SERIAL_*` are compiled out in PROD to eliminate UART ~1 mA draw. Sleep: 5 min (300 s) for all RUN_MODEs. **Battery operation:** ensure `DETACH_USB_BEFORE_SLEEP` is enabled (default in PROD) so sleep current reaches ~5–15 µA; otherwise ~10 mA USB leakage. At startup the sketch prints **Starting VBat=…**; below **3.4 V** the sleep interval is doubled (e.g. 600 s); below **3.2 V** (critical) the device skips TX and sleeps 600 s to avoid brown-out during LoRa burst. **BOD33** is set to 2.7 V for clean shutdown during TX. Ensure a 1000µF capacitor on the supply to prevent false resets during the SF7 TX spike. DS18B20 read has a **2 s timeout**; on timeout temperature is sent as error (0xFFFF). **Spreading factor:** Default SF7; if link is dead three times in a row the next uplink uses SF8 for one cycle; if DeviceTimeReq fails 3× the next FPort 4 time request uses SF9. **Join:** 90 s watchdog budget for join; after 3 consecutive join-phase watchdog triggers, 1-hour hibernation. **EV_JOIN_FAILED** uses adaptive backoff: 10 min, then 30 min, then 1 h. **Cold Boot test:** pull battery, reattach; verify device joins at SF7 and resumes correct seqnoUp from flash (no drift); after 3× EV_LINK_DEAD the next cycle may use SF8.

**Session resumption / frame counter:** If the NS rejects uplinks after restore (e.g. frame counter errors in TTN Live Data), enable **Resend Frame Counter** or **Relax Frame Counter** (or equivalent) in the TTN Console for the device so the device can resume with the persisted SeqNo instead of re-joining every wake. Stored session with DevAddr 0 is treated as invalid and forces a new join. All devices in this application use uniform v2.8 logic; no versioning bytes in the payload.
